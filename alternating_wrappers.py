...
class PCSEnvWrapper(gym.Wrapper):
    ...
    def _unnormalize_iso_action(self, normalized_action):
        ...
        # Check if dimensions don't match and handle appropriately
        if isinstance(normalized_action, np.ndarray) and low.shape != normalized_action.shape:
            self.logger.warning(f"Action shape mismatch: expected {low.shape}, got {normalized_action.shape}")
            
            # If the normalized action has fewer dimensions than the action space
            if len(normalized_action) < len(low):
                # Pad with zeros or extend to match expected dimensions
                padded_action = np.zeros(low.shape)
                padded_action[:len(normalized_action)] = normalized_action
                normalized_action = padded_action
                self.logger.info(f"Padded normalized action to shape {normalized_action.shape}")
            
            # If the normalized action has more dimensions than the action space
            elif len(normalized_action) > len(low):
                # Truncate to match expected dimensions
                normalized_action = normalized_action[:len(low)]
                self.logger.info(f"Truncated normalized action to shape {normalized_action.shape}")
        
        # Standard linear rescaling from [-1, 1] to [low, high]
        unnormalized_action = low + (normalized_action + 1.0) * 0.5 * (high - low)
        
        # Add detailed INFO-level logging for ISO action rescaling
        if isinstance(normalized_action, np.ndarray) and len(normalized_action) > 0:
            # Log buy price (first element)
            self.logger.info(f"Rescaled ISO buy price from {normalized_action[0]:.4f} to {unnormalized_action[0]:.4f} [range: {low[0]:.1f}-{high[0]:.1f}]")
            
            # Log sell price (second element) if it exists
            if len(normalized_action) > 1:
                self.logger.info(f"Rescaled ISO sell price from {normalized_action[1]:.4f} to {unnormalized_action[1]:.4f} [range: {low[1]:.1f}-{high[1]:.1f}]")
            
            # Log dispatch (third element) if it exists
            if len(normalized_action) > 2:
                self.logger.info(f"Rescaled ISO dispatch from {normalized_action[2]:.4f} to {unnormalized_action[2]:.4f} [range: {low[2]:.1f}-{high[2]:.1f}]")
        else:
            self.logger.info(f"Rescaled ISO action from {normalized_action:.4f} to {unnormalized_action:.4f} [range: {low:.1f}-{high:.1f}]")
            
        self.logger.debug(f"Unnormalized ISO action from {normalized_action} to {unnormalized_action}")
        return unnormalized_action
