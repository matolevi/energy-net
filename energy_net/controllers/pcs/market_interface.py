...
class MarketInterface:
    ...
    def calculate_market_position(
        ...
        Args:
            ...
            energy_change: Energy change in battery (positive for charging)
            
        Returns:
            Dictionary with market position information
        """
        # Calculate net exchange with grid (negative = selling to grid, positive = buying from grid)
        # Energy balance: consumption - production - battery_energy_delta
        self.net_exchange = consumption - production - energy_change
        
        # Calculate financial impact
        if self.net_exchange > 0:  # Buying from grid
            self.revenue = -self.net_exchange * self.iso_sell_price
            if self.logger:
                self.logger.info(f"Buying {self.net_exchange:.2f} MWh from grid at ${self.iso_sell_price:.2f}/MWh = ${self.revenue:.2f}")
        else:  # Selling to grid
            self.revenue = -self.net_exchange * self.iso_buy_price
            if self.logger:
                self.logger.info(f"Selling {-self.net_exchange:.2f} MWh to grid at ${self.iso_buy_price:.2f}/MWh = ${self.revenue:.2f}")
        ...
        self.dispatch_cost = 0.0
        dispatch_thresholds = self.pcs_config.get('dispatch_cost', {}).get('thresholds', [])
        
        for threshold in dispatch_thresholds:
            level = float('inf') if threshold['level'] == 'inf' else float(threshold['level'])
            if self.dispatch <= level:
                self.dispatch_cost = threshold['rate'] * self.dispatch
                break
